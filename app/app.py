...
with st.sidebar:
    ...
    if not paper_loaded:
        ...
        if "last_answer" in st.session_state:
            ...
            answer_mode  = ans.get("mode", "extractive")

            if answer_mode == "generative":
                # Generative: full synthesized answer, show which model produced it
                st.markdown(f"""
                    <div class="answer-block">
                        <div class="answer-label ans">Answer</div>
                        <div class="answer-text">{ans.get('answer', '—')}</div>
                    </div>
                    <div style="margin-top:8px; font-size:0.72rem; color:#2a6496; opacity:0.7;">
                        ✦ Generated by {ans.get('model', 'Llama 3')} via Groq
                        · {ans.get('chunks_used', '—')} chunks retrieved
                    </div>
                """, unsafe_allow_html=True)

            else:
                # Extractive: span answer, confidence score, context snippet
                # Identical to what you had before — nothing broke
                st.markdown(f"""
                    <div class="answer-block">
                        <div class="answer-label ans">Answer</div>
                        <div class="answer-text">{ans.get('answer', '—')}</div>
    ...
    st.markdown("""
        <div style="font-size:0.7rem; color:#2a4a62; text-align:center; padding-top:4px;">
            Powered by ArXiv API · Groq · v1.1
        </div>
    """, unsafe_allow_html=True)
...
if st.session_state.get("loaded"):
    ...
    if st.session_state.get("analysis_done"):
        ...
        # One call — all four fields arrive together
        # This guarantees the domain in the badge matches the domain the
        # summary was written for. Single LLM call = no drift.
        if "analysis_cache" not in st.session_state:
            with st.spinner("Analysing paper with Llama 3…"):
                try:
                    resp = requests.get(f"{API_URL}/analysis")
                    resp.raise_for_status()
                    st.session_state["analysis_cache"] = resp.json()
                except Exception as e:
        ...
        # ── Row 1: Summary + right column (Domain + Contribution) ─────────────
        col_sum, col_right = st.columns([3, 2])
        ...
        with col_right:
            # Domain badge — new, was not shown before
            ...
            # Contribution — new, was not shown before
        ...
        if keywords:
            # keywords here is list[str] from the LLM
            # (paper_assistant converts to dicts for the /keywords endpoint,
            #  but /analysis returns raw strings — simpler for display)
            ...
            chips_html = "".join(
                f'<span class="chip">{kw}</span>' for kw in kw_list
            )
            st.markdown(f"""
        ...
        st.markdown("""
            <div style="
                ...
                color: #4a6f8a;
            ">
                💡 Use <strong style="color:#8bafc9;">Ask the Paper</strong>
                in the sidebar to query this paper's content.
            </div>
        """, unsafe_allow_html=True)
